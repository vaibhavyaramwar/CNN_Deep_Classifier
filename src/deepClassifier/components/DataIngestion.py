...
class DataIngestion:
    ...
    def download_files(self):
        ...
        if not os.path.exists(self.config.local_data_file):
            ...
            logger.info(
                f"File Download is Successful and filename : {fileName} info of file : {headers}"
            )
        else:
             logger.info(f"File already exists of size: {utility.get_size(Path(self.config.local_data_file))}")  
